...
export default function SpeakersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editId) {
      await fetch(`/api/admin/speakers/${editId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    } else {
      await fetch('/api/admin/speakers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    }
    setForm(empty);
    setEditId(null);
    setShowForm(false);
