          ...
          {workshops.map((ws) => {
            ...
            return (
              <StaggerItem key={ws.id}>
                ...
                {ws.description && (
                  <p className="text-xs text-muted line-clamp-2 leading-relaxed">{ws.description}</p>
                )}

                <TapButton
                  onClick={(e) => handleBook(e, ws.id)}
                  disabled={bookingInProgress === ws.id || (booked && booking?.status !== 'approved')}
                  className={`w-full rounded-2xl py-3 text-xs font-semibold transition-all duration-150 ${
                    booked
                      ? (status?.color || 'bg-green-500/15 text-green-400') + ' hover:bg-red-500/15 hover:text-red-400'
                      : 'btn-dark hover:opacity-90'
                  } disabled:opacity-50`}
                >
